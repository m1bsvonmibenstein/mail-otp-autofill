...
> **Status:** the native app is currently a **lifecycle spike** (native-messaging plumbing only, no IMAP yet). See Roadmap.

## Roadmap

- [ ] **Native app: IMAP IDLE watcher** — real code source (mailcow + any IMAP), keychain-backed credentials, multi-account.
...
- [ ] **Multi-account** — tag codes by which account they came from.
...
native-app/   Rust native-messaging host (spike -> IMAP daemon)
```
